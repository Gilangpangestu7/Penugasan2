...
if (isset($_GET['hapus'])) {
    ...
    $sql_check = "SELECT COUNT(*) as total FROM dokter WHERE deleted_at IS NULL AND id_poli = ?";
    ...
    if ($row['total'] > 0) {
        ...
        $sql = "UPDATE poli SET deleted_at = CURRENT_TIMESTAMP WHERE id=?";
        $stmt = $conn->prepare($sql);
...
$sql_poli = "SELECT p.*, COUNT(CASE WHEN d.deleted_at IS NULL THEN d.id END) as jumlah_dokter 
FROM poli p 
LEFT JOIN dokter d ON p.id = d.id_poli AND d.deleted_at IS NULL
